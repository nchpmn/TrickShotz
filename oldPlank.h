...
class Plank {
    public:
        // Coordinates of each end of the line
        Pos<int16_t> position1 = Pos<int16_t>(0, 0);
        Pos<int16_t> position2 = Pos<int16_t>(0, 0);
        ...
        Plank(int16_t x1, int16_t y1, int16_t x2, int16_t y2) :
            position1(x1, y1), position2(x2, y2) {
    ...
    private:
        ...
        // Calculate and store the normal vector components on creation
            ...
            // 1. Caluclate length of line
            Vector plankVector(position2.x - position1.x, position2.y - position1.y);
            float dx = x2 - x1;
            float dy = y2 - y1;
            float length = sqrt(dx*dx + dy*dy);

            // 2. Caluclate components of normal vector (will be calc normal vector using Vector)

            normalX = dy / length;
            normalY = -dx / length; // Negative sign for the Y component since we want perpendicular vector
            
            // 3. Ensure that the normal vector points outward
